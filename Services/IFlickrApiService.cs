...
public interface IFlickrApiService
{
    ...
    Task<List<FlickrPhoto>> GetForLocationAsync(double latitude, double longitude, int accuracy = 1, int page = 1,
        int perPage = 10);

    Task<List<FlickrPhoto>> GetMoreForLocationAsync(double latitude, double longitude, int accuracy = 1, int page = 1,
        int perPage = 10);
